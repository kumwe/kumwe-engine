...
Engine 1.x implements all five modules against frozen owner semantics, including
...
consumer verification qualifies each deployed release. Engine has no PHP namespace, Composer/PHP/Zend/PIE dependency,
...
Linux additionally tests an independent frozen C11 client against the current dynamic library. Linux TSan covers the shared
immutable-plan test; the separate PHP binding runs its own supported compatibility matrix. Every supported source consumer builds offline from the pinned dependency source closure.
...
Releases retain frozen ABI 1, all five modules and the complete quality workflow.
Core qualifies the exact deployment tuple and workload independently. Modules
